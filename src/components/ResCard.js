...
const ResCard = (props) => {
  const { resobj } = props;
  const { cloudinaryImageId, name, cuisines, costForTwo, locality, avgRating } =
    resobj?.info || {};
...
export const addDiscount = (ResCard) => {
  return (props) => {
    const{resobj} = props;
    return (
      <div>
        <label className="dis">
          {resobj?.info?.aggregatedDiscountInfoV3?.header} {" "}
          {resobj?.info?.aggregatedDiscountInfoV3?.subHeader}{" "}
        </label>
        <ResCard {...props} />
      </div>
    );
